...
const moveSideButtons = (e) => {
  if (!leftButton.value || !rightButton.value) return;
...
onMounted(() => {
  ...
  gsap.from(".hero-text", { opacity: 0 });

  gsap.from(".hero-btn-1", { y: 100, opacity: -5, duration: 1.1 });
  gsap.from(".hero-btn-2", { y: 100, opacity: -5, duration: 1.4 });

  gsap.fromTo(
    ".hero-text",
    { scale: 0.3, backgroundPosition: "100% 0" },
    { scale: 1, opacity: 1, duration: 2.5, ease: "power3.out", backgroundPosition: "0% 0" }
  );
...
onUnmounted(() => {
  window.removeEventListener("mousemove", moveSideButtons);
});

watch(locale, (newLocale, oldLocale) => {
  if (newLocale !== oldLocale) {
    const textContainer = document.querySelector(
      ".relative.z-10.text-center.flex-1.select-none .flex.justify-center.items-center"
    );
    if (!textContainer) return;

    const currentEl = Array.from(textContainer.children)
    ...
    if (currentEl) {
      gsap.to(currentEl, {
        ...
        onComplete: () => {
          nextTick(() => {
            const newEl = Array.from(textContainer.children)
              .find((c) => c.offsetParent !== null)
              ?.querySelector(".hero-text");
            if (newEl) {
              gsap.set(newEl, { opacity: 0, scale: 0.8 });
              gsap.to(newEl, { opacity: 1, scale: 1, duration: 0.5, ease: "back.out(1.7)" });
            }
          });
        },
      });
      ...
      >
        ...
        >
          <mask
            ...
            height="320"
          >
            <path d="M77.41 0.0271912H0V319.973H77.41V0.0271912Z" fill="white" />
          </mask>
          <g mask="url(#mask0_185_55)">
            <mask
              ...
              height="321"
            >
              <path d="M49.0263 0.543243V320.489H0.516071V0.543243H49.0263Z" fill="white" />
            </mask>
        ...
        <img
          ...
          alt="arrow-left"
          class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 ml-[-17px]"
        />
      </button>
      ...
      <div class="relative z-10 text-center flex-1 select-none">
        <div class="flex justify-center items-center">
          <div v-if="$i18n.locale === 'ru'">
            <h1 class="font-extrabold hero-text mx-auto text-center">
            ...
            <NuxtLink :to="localePath('/contact')">
              ...
              >
                ...
                <img
                  ...
                  :class="btn.isHover ? 'invert-0 brightness-0' : 'invert brightness-0'"
                />
              </button>
            </NuxtLink>
          </div>
      ...
      >
        ...
        >
          <mask
            ...
            height="320"
          >
            <path d="M78 0.0271912H0.589966V319.973H78V0.0271912Z" fill="white" />
          </mask>
          <g mask="url(#mask0_185_2)">
            <mask
              ...
              height="321"
            >
              <path d="M28.9736 320.489V0.543274H77.4839V320.489H28.9736Z" fill="white" />
            </mask>
        ...
        <img
          src="/images/arrow_left.webp"
          alt="arrow-right"
          class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 rotate-180 ml-[17px]"
        />
      </button>
    </div>
  </section>
